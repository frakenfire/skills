# StockPulse — 초보자를 위한 주식 신호 앱

주식 **완전 초보자**를 위한 모바일 우선 웹앱. 깔끔한 핀테크 UI에, 종목마다 **강세 · 중립 · 약세 신호**를 쉬운 말로 보여주고, **공유 카드**로 자연스럽게 퍼지게 설계했습니다. 수익화는 **프리미엄 구독(Freemium)**.
...
- **상태를 대신 요약**: 차트를 못 봐도 "강세 / 중립 / 약세 + 신호 강도 + 데모 백테스트 적중률"로 한눈에.
- **부담 없는 진입**: 무료로 핵심을 맛보고, 더 보고 싶을 때 월 9,900원.

## 법적 톤(중요)

규제 리스크를 낮추려고 **"사라/팔아라" 권유 표현을 쓰지 않습니다.** 모든 신호는 종목의 *현재 상태*를 강세/중립/약세로 **서술**할 뿐이며, 화면 곳곳에 "기술적 지표 기반 정보 제공이며 투자 권유가 아닙니다 · 판단과 책임은 본인" 고지를 둡니다. 개인 맞춤 종목 *추천*으로 보이지 않도록 카피를 의도적으로 두루뭉술하게 유지합니다. (실제 영업 전 법무 검토 권장)

## 주요 기능(이번 버전)

- **신호 엔진**: RSI(14) + 단기/장기 이동평균 크로스오버 → 강세/중립/약세 + 신호 강도 + 간이 백테스트 적중률(`src/data/signals.js`)
- **가격 알림(Pro)**: 목표가 도달 시 **실제 브라우저 알림** 발송(`src/data/alerts.js`, Web Notifications API)
- **첫 진입 온보딩**: 관심 종목 1~3개 선택으로 개인화 시작(`OnboardingSheet`)
- **정돈된 UI**: 이모지/그라데이션 배제, 라인 아이콘, 중성 팔레트 + 단일 액센트
