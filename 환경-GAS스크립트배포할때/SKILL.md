...
## 인증 영속화 — 새 환경에서 로그인 화면 다시 뜨게 하지 말 것

clasp 로그인/OAuth 동의 화면이 또 뜬다면 **토큰을 저장·복원하지 않은 것**이다. 인터랙티브 로그인은 한 번만 하면 된다.

1. OAuth 동의는 **최초 1회만.** 끝나면 `~/.clasprc.json`(refresh_token 포함)이 생긴다.
2. 이 파일을 **보존**하고, 새 환경(컨테이너 등)에선 작업 시작 전에 **그대로 복원**한다 → clasp + REST가 재인증 없이 동작.
3. `access_token`이 만료돼도 `refresh_token`으로 갱신하면 되므로 **사람 개입(로그인) 불필요.**
4. ⚠️ `refresh_token`은 자격증명 → **공개 레포에 커밋 금지.** 비공개 복원 경로(부트스트랩의 안전한 위치)에만 둔다.
5. 부트스트랩 절차에 "`~/.clasprc.json` 복원"을 포함시켜, 매번 새로 뜨는 환경에서도 로그인 화면이 안 뜨게 한다.

→ 로그인 URL/인증코드를 채팅에 붙여넣으라고 요구하는 흐름은 **토큰 미복원의 증상**이다. 정상 흐름은 복원된 `~/.clasprc.json` + refresh.
