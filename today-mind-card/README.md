...
## 컨셉 이미지 / 아트 (`src/assets/`)

사용자 제공 컨셉 무드보드를 WebP로 최적화(원본 ~14.5MB → 합계 ~204KB)해 사용합니다.
톤은 딥 틸그린(≈ `#114e48`) + 따뜻한 크림/골드 라이트 + 세이지 그린으로,
브랜드 Primary와 위로·희망 무드에 그대로 맞습니다.

| 파일 | 쓰임 | 연결 |
|---|---|---|
| `hero-holding-card.webp` | StartScreen 히어로 | 카드를 안고 있는 사람 (제품 컨셉) |
| `art-forest.webp` | 결과/상세 삽화 | `숲` 회복 |
| `art-star.webp` | 결과/상세 삽화 | `별` 희망 |
| `art-door.webp` | 결과/상세 삽화 | `문` 선택 |
| `art-seed.webp` | 결과/상세 삽화 | `씨앗` 다시 시작 |

아트가 매핑된 카드(`src/data/cardArt.ts`)를 뽑으면 결과·상세 화면에 삽화가 노출되고,
나머지 카드는 그라디언트+이모지 기본 비주얼로 폴백합니다. 아트 추가는 `cardArt.ts` 매핑만 늘리면 됩니다.
