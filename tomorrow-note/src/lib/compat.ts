...
const REASON: Record<Tag, string[]> = {
  same: [
    ...
    '같은 띠끼리는 말 안 해도 통하는 지점이 많아요.',
    '닮은 구석이 많아 부담 없이 편한 사이가 되기 쉬워요.',
  ],
  trine: [
    '전통 궁합에서 삼합이라 부르는 조합이에요. 애쓰지 않아도 결이 비슷해서 잘 맞아요.',
    '삼합 궁합이에요. 예로부터 찰떡궁합으로 꼽히던 조합이라 오늘도 기대해도 좋아요.',
    '삼합은 세 띠가 한 팀처럼 도는 관계예요. 함께면 시너지가 나요.',
    '결이 비슷해 오래 봐도 편한, 전통적으로 손꼽히는 궁합이에요.',
  ],
  union: [
    '육합이라 불리는 짝꿍 조합이에요. 자연스럽게 손발이 맞는 편이에요.',
    '전통 궁합으로 보면 육합 관계예요. 큰 노력 없이도 무난하게 잘 맞아요.',
    '육합은 서로 부족한 걸 채워주는 짝이에요. 균형이 잘 맞아요.',
    '옛말로 육합이라, 곁에 있으면 든든해지는 조합이에요.',
  ],
  clash: [
    '정면으로 마주보는 상충 궁합이에요. 부딪히기 쉽지만 그만큼 자극과 텐션이 있어요.',
    '상충 관계라 티키타카가 심할 수 있어요. 근데 그게 은근 재밌는 조합이에요.',
    '정반대라 서로 없는 걸 가졌어요. 부딪히면서도 배우는 사이예요.',
    '상충은 불꽃 튀는 관계예요. 밀당의 재미가 있는 조합이죠.',
  ],
  harm: [
...
const GOOD = [
  ...
  '작은 배려가 두 배로 돌아와요.',
  '같이 있으면 없던 힘도 생기는 사이예요.',
  '사소한 대화도 오래 이어지는 케미예요.',
  '서로의 페이스를 존중해주는 편이에요.',
  '오늘은 함께 정한 일이 특히 잘 굴러가요.',
];
const CAUTION = [
  '급하게 결론 내리면 오해가 생겨요.',
  ...
  '각자의 시간도 존중해주면 좋아요.',
  '한쪽만 애쓰면 서운함이 쌓일 수 있어요.',
  '말 안 해도 알겠지, 하는 게 오해의 시작이에요.',
  '오늘은 승부욕보다 양보가 편해요.',
  '피곤할 땐 대화를 잠깐 미뤄도 괜찮아요.',
];
const TIP = [
  '오늘은 먼저 안부를 건네보세요.',
  '같이 맛있는 거 하나 먹으면 풀려요.',
  ...
  '가벼운 산책 한 번 같이 어때요?',
  '오늘 있었던 일 하나를 공유해보세요.',
  '칭찬 한마디가 오늘따라 크게 와닿아요.',
  '먼저 웃어주면 분위기가 확 부드러워져요.',
  '짧은 안부 메시지 하나면 오늘은 충분해요.',
];
