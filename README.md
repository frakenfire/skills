...
반복 프롬프트로 토큰을 낭비하지 않기 위해 공통 운영 원칙은 별도 문서로 관리한다.

- [TOKEN_EFFICIENT_AGENT.md](./TOKEN_EFFICIENT_AGENT.md)

핵심 원칙:

1. 먼저 계획하고, 실제 repo 상태를 확인한 뒤 수정한다.
2. 정상 동작 중인 코드는 최소 범위로만 건드린다.
3. 전체 파일·긴 diff·반복 설명을 불필요하게 출력하지 않는다.
